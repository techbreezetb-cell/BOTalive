# bot.py - The final and fully functional bot code
import telebot
import sqlite3
...
def get_onboarding_channels():
    cursor.execute("SELECT channel_name, channel_id FROM channels WHERE type='admin'")
    return cursor.fetchall()

def get_all_joinable_channels():
    cursor.execute("SELECT channel_name, channel_id FROM channels WHERE type IN ('join', 'admin')")
    return cursor.fetchall()
...
def is_member_of_mandatory_channels(user_id):
    channels = get_onboarding_channels()
    if not channels:
        return True
...
def reply_to_user_keyboard(user_id):
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("Reply to this user", callback_data=f"reply_to_user_{user_id}"))
    return markup
...
def start_command(message):
    ...
    if mandatory_channels and not is_member_of_mandatory_channels(user_id):
        all_channels = get_all_joinable_channels()
        markup = InlineKeyboardMarkup()
        for name, channel_id in all_channels:
...
def callback_query(call):
    ...
    if get_mandatory_channels() and not is_member_of_mandatory_channels(user_id):
        all_channels = get_all_joinable_channels()
        markup = InlineKeyboardMarkup()
        for name, channel_id in all_channels:
            markup.add(InlineKeyboardButton(f"Join {name}", url=f"https://t.me/{channel_id.replace('@', '')}"))
        markup.add(InlineKeyboardButton("✅ I have joined", callback_data='check_join'))
        bot.edit_message_text("Please join the following channels to continue:", call.message.chat.id, call.message.message_id, reply_markup=markup)
    ...
    if call.data == 'check_join':
        if is_member_of_mandatory_channels(user_id):
            social_links = get_social_links()
            if social_links and not has_clicked_social(user_id):
                markup = InlineKeyboardMarkup()
                for social_id, description, link in social_links:
            ...
            bot.answer_callback_query(call.id, "You have not joined all required channels yet!")
            bot.send_message(user_id, "Join the required channels and click 'I have joined' again to continue.")
    ...
    # ==== ADMIN PANEL CALLS ====
        ...
        if not is_admin(user_id): return
        msg = bot.send_message(user_id, "To add a channel, use `/addchannel <name> <channel_id> <type>`.\n\n**<type>** can be `join` or `admin`. Only `admin` channels are mandatory for users to join.")
        ...
        if not is_admin(user_id): return
        user_id_to_reply = int(call.data.split('_')[3])
        msg = bot.send_message(user_id, f"Please send your reply to user {user_id_to_reply}:")
        bot.register_next_step_handler(msg, process_admin_reply, user_id_to_reply)
...
def handle_withdrawal_request(call):
    ...
    if not wallet_address:
        ...
        bot.send_message(user_id, "❌ **Withdrawal Failed:** Please set your wallet address in the 'My Account' section to proceed.", parse_mode='Markdown')
        return
    ...
    if balance < min_amount:
        ...
        bot.send_message(user_id, f"❌ **Withdrawal Failed:** You need a minimum of {min_amount} {currency} to withdraw. Your current balance is {balance} {currency}.", parse_mode='Markdown')
        return
...
def process_support_message(message):
    ...
    bot.send_message(OWNER_ID, f"**New Support Message from {get_user_link(user_id)}**:\n\n`{support_message}`", parse_mode='Markdown', reply_markup=reply_to_user_keyboard(user_id))

def process_admin_reply(message, user_id_to_reply):
    if not is_admin(message.from_user.id): return
    reply_text = message.text
    try:
        bot.send_message(user_id_to_reply, f"**📢 Admin Reply:**\n\n{reply_text}", parse_mode='Markdown')
        bot.send_message(message.chat.id, f"✅ Reply sent successfully to user {user_id_to_reply}.", reply_markup=admin_main_menu_keyboard())
    except telebot.apihelper.ApiException as e:
        bot.send_message(message.chat.id, f"❌ Error sending reply: {e}", reply_markup=admin_main_menu_keyboard())
