# bot.py - Final, Fixed, and Fully Functional Version with New User Notification
import telebot
import sqlite3
...
def start_command(message):
    ...
    # Check if the user is new before adding them to the database
...
def callback_query(call):
    ...
    # ==== ADMIN PANEL CALLS ====
        ...
        if withdrawal:
            ...
            cursor.execute("UPDATE withdrawals SET status=?, paid_at=? WHERE id=?", (new_status, datetime.now().isoformat(), withdrawal_id))
            conn.commit()
